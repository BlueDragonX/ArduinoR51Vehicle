...
test(ClimateTempStateEventTest, Setters) {
    ...
    SystemEvent expect(Event::CLIMATE_TEMP_STATE, {0x40, 0x41, 0x49, 0x01});
...
test(ClimateAirflowStateEventTest, Setters) {
    ...
    SystemEvent expect(Event::CLIMATE_AIRFLOW_STATE, {0x07, 0x09});
    assertPrintableEqual(event, expect);
}
...
test(ClimateSystemStateEventTest, SetOff) {
    ...
    SystemEvent expect(Event::CLIMATE_SYSTEM_STATE, {0x00});
    assertPrintableEqual(event, expect);
}
...
test(ClimateSystemStateEventTest, SetAutoAC) {
    ...
    SystemEvent expect(Event::CLIMATE_SYSTEM_STATE, {0x05});
    assertPrintableEqual(event, expect);
}
...
test(ClimateSystemStateEventTest, SetDefrostACDual) {
    ...
    SystemEvent expect(Event::CLIMATE_SYSTEM_STATE, {0x0F});
    assertPrintableEqual(event, expect);
}
